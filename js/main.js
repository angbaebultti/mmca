document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!isMobile480) {
    ...
    shopTL.to(
      ".p",
      { opacity: 0.7, scale: 0.6, duration: 0.25, ease: "power1.out" },
      0,
    );
    qsa(".p").forEach((el, i) => {
      shopTL.to(
    ...
    shopTL.to(
      ".glow_bg",
      { opacity: 0.7, scale: 1.8, filter: "blur(80px)", duration: 0.5 },
      0.25,
    );
    shopTL.to(
      ".glass_box",
      { scale: 1.04, z: 80, duration: 0.8, ease: "power3.out" },
      0.25,
    );
  }
